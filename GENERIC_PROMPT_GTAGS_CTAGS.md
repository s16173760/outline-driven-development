...
**Selection guide:** Discovery → fd | Code pattern → ast-grep | Simple edit → srgn | Text → rg | Symbol nav → global/ctags | Scope → tokei | VCS → git-branchless | JSON → jql (default), jaq (jq-compatible/complex)

**Workflow:** fd (discover) → gtags/ctags (index) → global (navigate) → ast-grep/rg (search) → Edit (transform) → git (commit) → git-branchless (manage)
